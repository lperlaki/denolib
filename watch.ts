...
async function* run(
    ...
    while (!state.abort) {
        let waitTime = Math.max(
            0,
            options.interval - (Date.now() - lastStartTime)
        );
...
function makeFilter({ test, ignore, ignoreDotFiles }: Options) {
    ...
    const ignoreRegex =
        typeof ignore === 'string' ? new RegExp(ignore) : ignore;
    return function filter(f: any, path: string) {
        if (ignoreDotFiles) {
...
async function walk(
    ...
    targets: (string | any)[],
    followSymlink: boolean,
    filter: (f: any, path: string) => boolean,
    changes: Changes
): Promise<void> {
    const promises = [];
    for (let f of targets) {
        ...
        if (info.isDirectory()) {
            ...
            promises.push(
                walk(prev, curr, files, followSymlink, filter, changes)
            );
        } else if (info.isFile()) {
            if (curr[path]) {
                continue;
...
async function statTraverse(path: string): Promise<any> {
...
function statTraverseSync(path: string): any {
