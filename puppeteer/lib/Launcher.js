...
const mkdtempAsync = () => Deno.makeTempDir({
  prefix: 'puppeteer_dev_profile-'
});
const removeFolderAsync = dir => Deno.remove(dir, {
  recursive: true
});
...
class Launcher {
  ...
  async launch(options = {}) {
    ...
    if (!chromeArguments.some(arg => arg.startsWith('--user-data-dir'))) {
      temporaryUserDataDir = await mkdtempAsync();
    ...
    const chromeProcess = Deno.run({
      args: [chromeExecutable, ...chromeArguments],
      env,
      stdin: 'piped',
      stdout: 'piped',
      stderr: 'piped',
